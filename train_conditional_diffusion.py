"""Train class conditional diffusion model."""
...
def train(rank, world_size, config):
    ...
    if rank % world_size == 0:
        ...
        summary_string = f"condition_diffusion_{config['dataset']}_{current_time}"
    ...
    # model setup
    net_model = UNet(T=config["T"], num_classes=config["num_classes"], ch=config["channel"], ch_mult=config["channel_mult"], num_res_blocks=config["num_res_blocks"], dropout=config["dropout"]).to(rank)
    if rank % world_size == 0:
        ema_model = copy.deepcopy(net_model)
        ema_sampler = GaussianDiffusionSampler(ema_model, config["beta_1"], config["beta_T"], config["T"], w=config["w"]).to(rank)
    ...
    for e in range(config["epoch"]):
        with tqdm(dataloader, dynamic_ncols=True) as tqdmDataLoader:
            ...
            for images, labels in tqdmDataLoader:
                ...
                optimizer.zero_grad()
                x = images.to(rank)
                labels = labels.to(rank) + 1
                if np.random.rand() < 0.1:
                    labels = torch.zeros_like(labels).to(rank)
                loss = ddp_trainer(x, labels).mean()
                loss.backward()
            ...
            with torch.no_grad():
                ...
                for i in range(1, config["batch_size"] + 1):
                    ...
                    if i % step == 0:
                        if k < 10 - 1:
                            k += 1
                labels = torch.cat(labelList, dim=0).long().to(rank) + 1
                noisyImage = torch.randn(size=[config["batch_size"], 3, config["img_size"], config["img_size"]], device=rank)
                sampledImgs = ema_sampler(noisyImage, labels=labels, sample_method="ddpm")
