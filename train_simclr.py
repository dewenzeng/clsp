...
def train(config):
    ...
    summary_string = f"simclr_{config['dataset']}_{config['backbone']}_bs-{config['batch_size']}_temp-{str(config['temperature'])}_{current_time}"
    ...
    for e in range(config["epoch"]):
        with tqdm(dataloader, dynamic_ncols=True) as tqdmDataLoader:
            ...
            for images, labels in tqdmDataLoader:
                ...
                loss = info_nce_loss_with_label(features, temperature=config['temperature'])
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
