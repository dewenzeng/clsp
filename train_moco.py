...
def mocov2plus_loss_func(
    ...
    Compute the scores based on https://github.com/DTennant/CL-Visualizing-Feature-Transformation/blob/c53d1befa31d27fc6db63009174355f3095009f5/memory/mem_moco.py#L69
...
def train(config: Dict):
    ...
    summary_string = f"moco_{config['dataset']}_{config['backbone']}_bs-{config['batch_size']}_temp-{str(config['temperature'])}_{current_time}"
    ...
    for e in range(config["epoch"]):
        with tqdm(dataloader, dynamic_ncols=True) as tqdmDataLoader:
            ...
            for images, _ in tqdmDataLoader:
                ...
                q1, q2, k1, k2 = model(images[0], images[1])
                queue = model.module.queue.clone().detach() if config["use_dp"] else model.queue.clone().detach()
                loss1 = mocov2plus_loss_func(q1, k2, queue[1], config["temperature"])
                loss2 = mocov2plus_loss_func(q2, k1, queue[0], config["temperature"])
                ...
                if n_iter % config['log_interval'] == 0:
                    ...
                    writer.add_scalar('loss_moco', loss.item(), global_step=n_iter)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--config", type=str, default="configs/moco_cifar10.yaml")
