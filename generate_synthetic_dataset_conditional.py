...
from models.unet_condition import UNet
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ddim_eta', default=1.0, type=float, help='ddim eta')
    parser.add_argument('--sample_method', default='ddpm', type=str, help='diffusion sample method', choices=['ddpm', 'ddim'])
    ...
    for candidate_idx in range(args.num_candidates):
        ...
        with torch.no_grad():
            ...
            model = UNet(T=config["T"], num_classes=config["num_classes"], ch=config["channel"], ch_mult=config["channel_mult"], num_res_blocks=config["num_res_blocks"], dropout=0.)
            ckpt = torch.load(args.ckpt_path, map_location=device)
            model.load_state_dict(ckpt)
            print("model load weight done.")
            model.eval()
            sampler = GaussianDiffusionSampler(model, config["beta_1"], config["beta_T"], config["T"], w=config["w"]).to(device)
            sampler = torch.nn.DataParallel(sampler)
            for batch_idx, (anchor_image, label, idx) in enumerate(dataloader):
                print(f"sample {batch_idx+1}/{len(dataloader)}")
                labels = label.to(device) + 1
                # Sampled from standard normal distribution.
                noisyImage = torch.randn(size=[anchor_image.shape[0], 3, config["img_size"], config["img_size"]], device=device)
                sampledImgs = sampler(noisyImage, labels=labels, sample_method=args.sample_method, ddim_sampling_timesteps=args.ddim_sampling_timesteps, ddim_eta=args.ddim_eta)
        ...
        np.save(os.path.join(args.save_dir, f'generated_conditional_{args.sample_method}_{candidate_idx:03d}.npy'), generated_images)
    
    # combining all synthetic images.
    all_images = []
    for candidate_idx in range(args.num_candidates):
        all_images.append(np.load(os.path.join(args.save_dir, f'generated_conditional_{args.sample_method}_{candidate_idx:03d}.npy')))
    all_images = np.stack(all_images, axis=1)
    print(f'all_images:{all_images.shape}')
    np.save(os.path.join(args.save_dir, f"synthetic_conditional_{config['dataset']}_{args.sample_method}_{args.num_candidates}_candidates.npy"), all_images)
